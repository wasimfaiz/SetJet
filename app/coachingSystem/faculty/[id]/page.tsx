...
const FacultyViewPage = () => {
  ...
  const fetchFaculty = async (id: any) => {
    try {
  ...
  const handleEdit = (item: any) => {
    ...
    router.push(`/coachingSystem/faculty/add?id=${item._id}`);
  };
      ...
      {selectedItem ? (
        <>
          <View
            ...
            handleEdit={
              checkButtonVisibility(permissions, "faculty", "edit")
                ? handleEdit
                : undefined
            }
          />
          <div className="overflow-x-auto my-5 bg-white flex flex-col justify-center py-5 shadow-lg rounded-lg">
            ...
            {selectedItem?.permissions !== "all" ? (
              <table className="table-auto border-collapse border border-gray-300 w-4/5 text-left mx-5">
                <thead>
