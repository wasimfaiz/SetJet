// src/app/(your-folder)/employee-login/page.tsx
"use client";
...
import { EmployeeLoginPayload, login } from "@/app/utils/auth";
...
export default function Auth() {
  ...
  const handleSubmit = async (values: EmployeeLoginPayload) => {
    setLoading(true);
    try {
      const data = await login("EMPLOYEE", {
        email: values.email,
        password: values.password,
      });

      // data has { token, employeeEmail }
      localStorage.setItem("token", (data as any).token);
      localStorage.setItem("email", (data as any).employeeEmail);
