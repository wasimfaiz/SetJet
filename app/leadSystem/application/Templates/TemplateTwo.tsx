...
/* ────────────────────── BULLET RENDERER (same as TemplateOne) ────────────────────── */
const renderBullet = (txt?: string) => {
  ...
  return points.map((point, i) => (
    <div key={i} className="flex items-start gap-0 pl-1 page-break-inside-avoid">
...
export default function TemplateTwo({ formData }: TemplateTwoProps) {
  if (!formData) {
    return <p className="text-center text-gray-500 mt-20">No CV data available</p>;
  }
    ...
    <>
      ...
      >
        <div className="flex">
          {/* ────────────────────── LEFT SIDEBAR ────────────────────── */}
          <aside
            className="w-2/5 p-6 flex flex-col items-center page"
            style={{ backgroundColor: "#1b232e", color: "#fff" }}
          >
            {/* Avatar */}
            ...
            {formData.jobTitle && (
              <p className="text-sm text-blue-400 print:text-black">{formData.jobTitle}</p>
            )}
  ...
  <div className="flex items-center gap-2 print:text-black print:no-underline mt-2">
    ...
    <span className="break-words leading-none mb-4">{formData.email}</span>
            ...
            {Array.isArray(formData.skills) && formData.skills.length > 0 && (
              <section className="w-full text-sm mt-6 page">
                <h2 className="text-md font-bold mb-2 print:text-black">SKILLS</h2>
            ...
            {formData.languages && formData.languages.length > 0 && (
              <section className="w-full text-sm mt-6 page">
                <h2 className="text-md font-bold mb-2 print:text-black">LANGUAGES</h2>
                <ul className="list-none space-y-1">
            ...
            {formData.hobbies && formData.hobbies.length > 0 && (
              <section className="w-full text-sm mt-6 page">
                <h2 className="text-md font-bold mb-2 print:text-black">HOBBIES</h2>
                <ul className="list-none space-y-1">
                  {formData.hobbies.map((hobby: string, i: number) => (
                    <li key={i} className="flex items-center print:text-black">
                      <span className="w-5 text-center">•</span>
                      <span className="flex-1 break-words">{hobby}</span>
                    </li>
                  ))}
                </ul>
              </section>
          ...
          {/* ────────────────────── RIGHT COLUMN ────────────────────── */}
          <main className="w-3/5 p-8 page">
            {/* PROFILE */}
            {formData.about && (
              <section className="mb-6 page">
                <h2 className="text-lg font-bold mb-2 print:text-black">PROFILE</h2>
                <p className="print:text-black break-words hyphens-auto">{formData.about}</p>
              </section>
            )}
            ...
            {Array.isArray(formData.workExperience) && formData.workExperience.length > 0 && (
              <section className="mb-6 page">
                <h2 className="text-lg font-bold mb-3 print:text-black">WORK EXPERIENCE</h2>
                {formData.workExperience.map((w: any, i: number) => (
                  <div key={i} className="mb-6 page">
                    <p className="font-bold print:text-black">{w.position}</p>
                    <p className="font-medium print:text-black">{w.company}</p>
            ...
            {Array.isArray(formData.education) && formData.education.length > 0 && (
              <section className="mb-6 page">
                <h2 className="text-lg font-bold mb-3 print:text-black">EDUCATION</h2>
