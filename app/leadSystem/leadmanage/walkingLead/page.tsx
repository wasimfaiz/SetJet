...
const LeadsPage = () => {
  ...
  const [statusFilter, setStatusFilter] = usePersistentState("empLead-filter-status",  "");
  const [interestedFollowClients, setInterestedFollowClients] = useState<any[]>([]);
  const [searchQuery, setSearchQuery] = usePersistentState("empLead-filter-search",  "");
  ...
  // Filter States
  const [assigned, setAssigned] = usePersistentState("empLead-filter-assign",  "UNASSIGNED");
  const [course, setCourse] = usePersistentState("empLead-filter-course",  "");
  const [selectedEmployee, setSelectedEmployee] = usePersistentState("empLead-filter-employee",  "");
  const [date, setDate] = usePersistentState("empLead-filter-date",  "");
  const [calledAtDate, setCalledAtDate] = usePersistentState("empLead-filter-call",  "");
  ...
  const fetchTotalByStatus = async (page = 1, status: string, search: string) => {
    try {
      setLoading(true);
      let url = `/api/leads/student-leads/total?page=${page}&status=${status}&searchTerm=${search}&limit=${pageLimit}&assigned=${assigned}&assignId=${selectedEmployee}&date=${date}&calledAtDate=${calledAtDate}&course=${course}&category=WALKING`;
  ...
  }, [searchQuery, statusFilter, assigned, selectedEmployee, date, calledAtDate, course, pageLimit]);

  // Update lead details and modal clients when selectedItem, currentPage, searchQuery, statusFilter, or isEditing change
  useEffect(() => {
    fetchClientsByStatus(statusFilter, searchQuery, currentPage);
  }, [selectedItem, currentPage, searchQuery, statusFilter]);
  ...
  return (
    <div className="container">
      ...
      {statusOptions.map((status) => {
          ...
          if (status === "CONVERTED")
            statusColor = "rounded-lg bg-parrotgreen text-deepblue text-sm font-medium hover:bg-green-100 hover:text-white";
          if (status === "PAYMENT MODE")
            statusColor = "rounded-lg bg-green-800 text-green-900 text-sm font-medium hover:bg-green-400 hover:text-white";
          if (status === "INTERESTED")
            statusColor = "rounded-lg bg-green-500 text-green-900 text-sm font-medium hover:bg-green-900 hover:text-white";
          if (status === "NOT INTERESTED")
            statusColor = "rounded-lg bg-red-500 text-red-900 text-sm font-medium hover:bg-red-900 hover:text-white";
          if (status === "DNP")
            statusColor = "rounded-lg bg-orange-600 text-orange-900 text-sm font-medium hover:bg-orange-700 hover:text-white";
          if (status === "FOLLOW UP")
            statusColor = "rounded-lg bg-red-400 text-deepblue text-sm font-medium hover:bg-red-500 hover:text-white";
          if (status === "SWITCH OFF")
            statusColor = "rounded-lg bg-blue-400 text-deepblue text-sm font-medium hover:bg-blue-600 hover:text-white";
          if (status === "CALL DISCONNECTED")
            statusColor = "rounded-lg bg-purple-400 text-purple-700 text-sm font-medium hover:bg-purple-600 hover:text-white";
          if (status === "OTHERS")
            statusColor = "rounded-lg bg-teal-500 text-teal-800 text-sm font-medium hover:bg-teal-700 hover:text-white";
          if (status === "TOTAL CALLS")
            statusColor = "rounded-lg bg-green-800 text-green-200 text-sm font-medium hover:bg-green-700 hover:text-white";
          const countToShow = counts?.[status] || 0;
          return (
            <button
              key={status}
      ...
      {/* Filters */}
      <div className="my-4 grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-nowrap items-center gap-2 mx-2">
        {/* Search Input */}
        <input
          type="text"
          value={searchQuery}
          onChange={(e) => handleSearch(e.target.value)}
          placeholder="Search Name, Mobile, or Employee"
          className="border p-2 w-full rounded-md text-xs md:text-sm"
        />
        ...
        <div className="relative w-full">
          ...
          {assigned && (
            <button
              onClick={() => setAssigned("")}
              className="absolute right-8 top-2 text-gray-400 hover:text-red-500 text-xs"
            >
              ✕
            </button>
          )}
        ...
        <div className="relative w-full">
          <input
            ...
            className="px-2 py-2 border rounded-md w-full text-xs md:text-sm"
          />
          {date && (
            <button
              onClick={() => setDate("")}
              className="absolute right-8 top-3 text-gray-400 hover:text-red-500 text-sm"
            >
              ✕
            </button>
          )}
      ...
      {/* Action Buttons */}
      <div className="flex flex-wrap space-x-2 sm:space-x-4 items-center mb-3 mx-2">
        <button
          disabled={selectedRows.length <= 0}
          className={`py-2 px-3 rounded-md text-xs md:text-sm ${
        ...
        <button
          className="py-2 px-3 rounded-md bg-purple-500 text-white hover:bg-purple-800 text-xs md:text-sm"
        ...
        {detailedView && (
          <div className="relative w-full md:w-1/6">
            <span className="text-xs md:text-sm">Called At Date</span>
            <input
              type="date"
              value={calledAtDate}
            ...
            {calledAtDate && (
              <button
                ...
                className="absolute right-2 top-1 text-gray-400 hover:text-red-500 text-xs"
              >
                ✕
              </button>
            )}
      ...
      <div className="my-5 mx-2">
        <Table
          ...
          //@ts-ignore
          columns={detailedView ? detailedColumns : columnsModal}
