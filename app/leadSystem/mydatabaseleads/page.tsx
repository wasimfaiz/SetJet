...
const leadsOptions = [
  { value: "GROUND LEADS", label: "GROUND LEADS" },
...
const DatabaseContentPage = () => {
  ...
  const [searchQuery, setSearchQuery] = usePersistentState("mydatabaseleads-filter-search",  "");
  const [statusFilter, setStatusFilter] = usePersistentState("mydatabaseleads-filter-status",  "");
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const { activeTab, changeTab } = usePersistentTab("GROUND LEADS");
  ...
  const [lookingFor, setLookingFor] = usePersistentState("mydatabaseleads-filter-course",  "");
  ...
  const fetchLeads = async (status: string, currentPage: number) => {
    ...
    try {
      ...
      let url = `/api/leads/student-leads?assignId=${employeeId}&status=${status}&page=${currentPage}&limit=${limit}&searchTerm=${searchQuery}&course=${lookingFor}&sort=sort`;
    ...
    if (selectedItem) {
      ...
      try {
        const response = await apiClient.put(
          `/api/leads/student-leads?id=${selectedItem._id}`,
          payload
        );
  ...
  const handleTransfer = async (transferTo: { id: string }) => {
    ...
    try {
      // 4) Update
      await apiClient.post("/api/leads/student-leads/transfer", {
        ids: leadIds,
        ...payload
      });
  ...
  const handleEmployee = async (empId: string, empName: string) => {
    ...
    try {
      const response = await apiClient.post(`/api/leads/student-leads/transfer`, payload);
      if (response.status === 200) {
        fetchLeads(statusFilter, currentPage)
...
<div className="container my-10">
  ...
  <div className="flex w-full mt-3 mx-2 gap-2">
    {leadsOptions.map((lead, index) => (
      <div
        key={index}
        onClick={() => handleNavigation(lead.value)}
        className={`px-3 py-1 rounded-lg ${
          lead.value === "GROUND LEADS"
            ? "bg-green-400 hover:bg-green-900"
            : "bg-red-400 hover:bg-red-500"
        } text-white text-xs font-medium shadow-sm cursor-pointer ${
          activeTab === lead.value ? "outline outline-1 outline-deepblue" : ""
        }`}
      >
        {lead.value}
      </div>
