...
import { faEye, faPhone, faTrashCan } from "@fortawesome/free-solid-svg-icons";
...
const columns = [
  ...
  { header: "Transfer to", accessor: "transferTo.name", type: "transfer" },
...
const DatabaseContentPage = () => {
  ...
  const fetchStudentDatabasebyDatabaseId = async (page = currentPage, status="ALL", searchTerm="") => {
    ...
    try {
      const response = await apiClient.get(
        `/api/database/student-database?databaseId=${id}&assignId=${employeeId}&page=${page}&limit=${pageLimit}&status=${status}&searchTerm=${searchTerm}`
      );
        if(status === "ALL")
        {
  ...
  }, [employeeId, pageLimit, currentPage]);
