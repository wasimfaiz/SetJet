...
function CollegeAddPageContent() {
  ...
  const { collegeId, courseId } = params as { collegeId: string; courseId: string };
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiClient.get(`/api/college/${collegeId}/course/${courseId}/courses?subCourseId=${subCourseId}`)
      .then((res) => {
        const c = res.data.subCourse;
        const base = { ...c };
        reset({ ...base });
      })
      .finally(() => setLoading(false));
  }, [collegeId, subCourseId, reset]);

  const onSubmit = async (data: any) => {
    const method = subCourseId ? "PUT" : "POST";
    const url = `/api/college/${collegeId}/course/${courseId}/courses`;
    await fetch(url + (subCourseId ? `?subCourseId=${subCourseId}` : ""), {
      method,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...data }),
    });
    router.push(`/leadSystem/college/${collegeId}/${courseId}`);
  };
  ...
  return (
    <FormProvider {...methods}>
      <div className="container mx-auto p-4">
        ...
        >
          {/* Stream Name with Publish */}
          <FieldWithPublish label="Stream Name" publishName="name_publish">
            <Controller
              name="name"
              control={control}
              rules={{ required: 'Stream Name is required' }}
